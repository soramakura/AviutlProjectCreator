...
)
{
    ...
    line = std::regex_replace(line, std::regex(R"(%DATEISO%)"), std::format
        ( "{:0>4}-{:0>2}-{:0>2}\n"
    ...
    line = std::regex_replace(line, std::regex(R"(%YEAR%)"  ), std::format("{:0>4}", timeStruct.tm_year + 1900));
    line = std::regex_replace(line, std::regex(R"(%MONTH%)" ), std::format("{:0>2}", timeStruct.tm_mon  + 1   ));
    line = std::regex_replace(line, std::regex(R"(%DAY%)"   ), std::format("{:0>2}", timeStruct.tm_mday       ));
    line = std::regex_replace(line, std::regex(R"(%HOUR%)"  ), std::format("{:0>2}", timeStruct.tm_hour       ));
    line = std::regex_replace(line, std::regex(R"(%MINUTE%)"), std::format("{:0>2}", timeStruct.tm_min        ));
    line = std::regex_replace(line, std::regex(R"(%SECOND%)"), std::format("{:0>2}", timeStruct.tm_sec        ));
    line = std::regex_replace(line, std::regex(R"(%TIME_T%)"), std::format("{:0>1}", currentTime              ));
